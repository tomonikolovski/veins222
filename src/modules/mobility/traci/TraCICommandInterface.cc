...
double TraCICommandInterface::getDistance(const Coord& p1, const Coord& p2, bool returnDrivingDistance) {
	uint8_t variable = DISTANCE_REQUEST;
	       ...
	       // std::cout<<static_cast< int >( variable )<<endl;
	std::string simId = "sim0";
	uint8_t variableType = TYPE_COMPOUND;
	//std::cout<<static_cast< int >( variableType )<<endl;
	int32_t count = 3;
	//std::cout<<count<<endl;
...
//	double distance1; buf >> distance1;
//	std::cout<<"buf: "<<distance1<<endl;
	uint8_t cmdLength_resp; buf >> cmdLength_resp; //std::cout<<buf.str().length()<<endl;
	//std::cout<<"cmdL: "<<static_cast< int >( cmdLength_resp )<<endl;
//	double distance2; buf >> distance2;
//	    std::cout<<distance2<<endl;
	uint8_t commandId_resp; buf >> commandId_resp; ASSERT(commandId_resp == RESPONSE_GET_SIM_VARIABLE);
	//std::cout<<"commandId: "<<static_cast< int >( commandId_resp )<<endl;
//	double distance2; buf >> distance2;
//	        std::cout<<distance2<<endl;
	uint8_t variableId_resp; buf >> variableId_resp; ASSERT(variableId_resp == variable);
	//std::cout<<"variableId: "<<static_cast< int >( variableId_resp )<<endl;
//	double distance4; buf >> distance4;
//	        std::cout<<distance4<<endl;
	std::string simId_resp; buf >> simId_resp; ASSERT(simId_resp == simId);
	//std::cout<<simId_resp<<endl;
//	double distance5; buf >> distance5;
//	        std::cout<<distance5<<endl;
	uint8_t typeId_resp; buf >> typeId_resp; ASSERT(typeId_resp == TYPE_DOUBLE);
	//std::cout<<"typeId: "<<static_cast< int >( typeId_resp )<<endl;
