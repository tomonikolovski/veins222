...
#include "TraCIScenarioManager2.h"
#include "TraCICommandInterface.h"
#include "TraCIConstants.h"
#include "TraCIMobility2.h"
...
using Veins::TraCIScenarioManager2;
using Veins::TraCIBuffer;
using Veins::TraCICoord;

Define_Module(Veins::TraCIScenarioManager2);

TraCIScenarioManager2::TraCIScenarioManager2() :
		myAddVehicleTimer(0),
		mobRng(0),
		connection(0),
...
void TraCIScenarioManager2::initialize(int stage) {
...
//	}
	...
	MYDEBUG << "initialized TraCIScenarioManager2" << endl;
}

void TraCIScenarioManager2::init_traci() {
	{
...
void TraCIScenarioManager2::finish() {
	if (connection) {
...
void TraCIScenarioManager2::handleMessage(cMessage *msg) {
...
void TraCIScenarioManager2::handleSelfMsg(cMessage *msg) {
	...
	error("TraCIScenarioManager2 received unknown self-message");
}



// name: host;Car;i=vehicle.gif
//void TraCIScenarioManager2::addModule(std::string nodeId, std::string type, std::string name, std::string displayString, const Coord& position, std::string road_id, double speed, double angle) {
//
...
cModule* TraCIScenarioManager2::getManagedModule(std::string nodeId) {
	if (hosts.find(nodeId) == hosts.end()) return 0;
	return hosts[nodeId];
}

bool TraCIScenarioManager2::isModuleUnequipped(std::string nodeId) {
	if (unEquippedHosts.find(nodeId) == unEquippedHosts.end()) return false;
	return true;
}

//void TraCIScenarioManager2::deleteManagedModule(std::string nodeId) {
...
bool TraCIScenarioManager2::isInRegionOfInterest(const TraCICoord& position, std::string road_id, double speed, double angle) {
...
uint32_t TraCIScenarioManager2::getCurrentTimeMs() {
	return static_cast<uint32_t>(round(simTime().dbl() * 1000));
}

void TraCIScenarioManager2::executeOneTimestep() {
...
void TraCIScenarioManager2::insertNewVehicle() {
	std::string type;
...
void TraCIScenarioManager2::insertVehicles() {
...
void TraCIScenarioManager2::subscribeToVehicleVariables(std::string vehicleId) {
...
void TraCIScenarioManager2::unsubscribeFromVehicleVariables(std::string vehicleId) {
	// subscribe to some attributes of the vehicle
	uint32_t beginTime = 0;
	uint32_t endTime = 0x7FFFFFFF;
	std::string objectId = vehicleId;
...
void TraCIScenarioManager2::processSimSubscription(std::string objectId, TraCIBuffer& buf) {
	...
	for (uint8_t j = 0; j < variableNumber_resp; ++j) {
		...
		if (variable1_resp == VAR_DEPARTED_VEHICLES_IDS) {
			...
			for (uint32_t i = 0; i < count; ++i) {
				...
				for (cModule::SubmoduleIterator iter(mod); !iter.end(); iter++) {
					cModule* submod = iter();
					TraCIMobility2* mm = dynamic_cast<TraCIMobility2*>(submod);
					if (!mm) continue;
...
void TraCIScenarioManager2::processVehicleSubscription(std::string objectId, TraCIBuffer& buf) {
...
void TraCIScenarioManager2::processSubcriptionResult(TraCIBuffer& buf) {
	uint8_t cmdLength_resp; buf >> cmdLength_resp;
...
//
//IPv4Address TraCIScenarioManager2::getIPAddressForID(std::string id){
