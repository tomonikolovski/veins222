 ...
 */
namespace Veins {
class TraCIMobility2 : public BaseMobility
{
	public:
		class Statistics {
			public:
		...
		TraCIMobility2() : BaseMobility(), isPreInitialized(false), manager(0), commandInterface(0), vehicleCommandInterface(0) {}
		~TraCIMobility2() {
		...
		virtual void nextPosition(const Coord& position, std::string road_id = "", double speed = -1, double angle = -1, TraCIScenarioManager2::VehicleSignal signals = TraCIScenarioManager2::VEH_SIGNAL_UNDEF);
		...
		virtual TraCIScenarioManager2::VehicleSignal getSignals() const {
		...
		virtual TraCIScenarioManager2* getManager() const {
			if (!manager) manager = TraCIScenarioManagerAccess2().get();
			return manager;
		}
	...
	protected:
		...
		TraCIScenarioManager2::VehicleSignal signals; /**<updated by nextPosition() */

		cMessage* startAccidentMsg;
		cMessage* stopAccidentMsg;
		mutable TraCIScenarioManager2* manager;
...
namespace Veins {
class TraCIMobilityAccess2
{
	public:
		TraCIMobility2* get(cModule* host) {
			TraCIMobility2* traci = FindModule<TraCIMobility2*>::findSubModule(host);
			ASSERT(traci);
			return traci;
		};
};
