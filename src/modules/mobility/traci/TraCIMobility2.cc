...
using Veins::TraCIMobility2;

Define_Module(Veins::TraCIMobility2);

const simsignalwrap_t TraCIMobility2::parkingStateChangedSignal = simsignalwrap_t(TRACI_SIGNAL_PARKING_CHANGE_NAME);
...
void TraCIMobility2::Statistics::initialize()
{
...
void TraCIMobility2::Statistics::watch(cSimpleModule& )
{
	WATCH(totalTime);
	WATCH(minSpeed);
	WATCH(maxSpeed);
...
void TraCIMobility2::Statistics::recordScalars(cSimpleModule& module)
{
...
void TraCIMobility2::initialize(int stage)
{
	if (stage == 0)
	{
...
void TraCIMobility2::handleSelfMsg(cMessage *msg)
{
...
void TraCIMobility2::preInitialize(std::string external_id, const Coord& position, std::string road_id, double speed, double angle)
{
...
void TraCIMobility2::nextPosition(const Coord& position, std::string road_id, double speed, double angle, TraCIScenarioManager2::VehicleSignal signals)
{
...
void TraCIMobility2::changePosition()
{
...
void TraCIMobility2::changeParkingState(bool newState) {
	isParking = newState;
	emit(parkingStateChangedSignal, this);
}

void TraCIMobility2::updateDisplayString() {
	ASSERT(-M_PI <= angle);
	ASSERT(angle < M_PI);
...
void TraCIMobility2::fixIfHostGetsOutside()
{
...
double TraCIMobility2::calculateCO2emission(double v, double a) const {
...
Coord TraCIMobility2::calculateAntennaPosition(const Coord& vehiclePos) const {
	Coord corPos;
